...
bl_info = {
    ...
    "blender": (2, 5, 5),
    "api": 31236,
    "location": "View3D > Specials > QuadricSimplification",
    "description": "Quadric based simplification",
    "warning": "",
    "wiki_url": "",
    "tracker_url": "",
    "category": "Mesh"}
...
class DecimateClass(bpy.types.Operator):
    bl_idname = 'mesh.decimate'
    bl_label = 'Quadric Decimate'
    bl_options = {'REGISTER', 'UNDO'}

    epsilon = bpy.props.FloatProperty(name='Epsilon', default=NODE_EQUIV, min = 0.0, max = 1.0)
    targetfaces = bpy.props.IntProperty(name='Target faces', default=DECIMATE_NUM, min = 0, max = 1000000)
    NODE_EQUIV = epsilon
    ...
    def execute(self, context):
        ...
        decimate(mymesh, self.targetfaces)
        mesh_to_blender(mymesh)
        
        return {'FINISHED'}

def decimate_button(self, context):
    self.layout.operator(DecimateClass.bl_idname, text="Quadric Simplification")

def register():
    bpy.utils.register_module(__name__)
    bpy.types.VIEW3D_PT_tools_objectmode.append(decimate_button)

def unregister():
    bpy.utils.unregister_module(__name__)
    bpy.types.VIEW3D_PT_tools_objectmode.remove(decimate_button)
