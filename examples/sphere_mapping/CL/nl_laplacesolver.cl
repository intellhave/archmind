...
__kernel void solve_conformal( 
             ...
             const __global unsigned int *bounds,
		     const __global unsigned int *verts,
             ...
             )
{
	...
	for( unsigned int i = bounds[id*2]; i < bounds[id*2+1]; ++i )
	{
        //neighbor vertex
		unsigned int nid = verts[i];
        c += v[nid] * w[i]; 
	}
		     ...
		     const int numOfVertices)
{
    ...
    int start = bounds[id*2];
    int end = bounds[id*2+1];
